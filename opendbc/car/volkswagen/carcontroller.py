  ...
  def update(self, CC, CS, now_nanos):
    ...
    if self.CP.openpilotLongitudinalControl:
      if self.frame % self.CCP.ACC_CONTROL_STEP == 0:
        if self.CP.flags & VolkswagenFlags.MEB:
          ...
          # MLB: gas override should be status 4 (background override), not 2 (passive), to keep the cluster HUD lead car
          gas_override = (self.CP.flags & VolkswagenFlags.MLB) and CC.enabled and CS.out.gasPressed
          acc_control = self.CCS.acc_control_value(CS.out.cruiseState.available, CS.out.accFaulted, CC.longActive, gas_override)
    ...
    if self.frame % self.CCP.ACC_HUD_STEP == 0 and self.CP.openpilotLongitudinalControl:
      if self.CP.flags & VolkswagenFlags.MEB:
        ...
        # MLB: gas override should keep HUD active (status 4)
        gas_override = (self.CP.flags & VolkswagenFlags.MLB) and CC.enabled and CS.out.gasPressed
        acc_hud_status = self.CCS.acc_hud_status_value(CS.out.cruiseState.available, CS.out.accFaulted, CC.longActive, gas_override)
